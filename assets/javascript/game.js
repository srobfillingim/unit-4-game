...
function initializeCharacter(chosenCharacter) {
  ...
  character.attackPoints = chosenCharacter.attackPoints;
...
function initializeDefender(chosenDefender) {
  ...
  defender.attackPoints = chosenDefender.attackPoints;
...
$(document).ready(function() {
  ...
  $("#attack").on("click", function() {
    ...
    if (characterSelected && defenderSelected && !gameOver) {
      ...
      defender.health = defender.health - character.counterAttackPower;
      $(".defender-character").children(".health").html(defender.health);
      $("#game-message").html("<p>You attacked " + defender.name + " for " + character.counterAttackPower + " damage.<p>");

      // User's counterAttackPower power increases
      character.counterAttackPower = character.counterAttackPower + character.attackPoints;

      // If defender is still alive, they counter attack the user
      if (defender.health > 0) {
        character.health = character.health - defender.attackPoints;
        $(".chosen-character").children(".health").html(character.health);

        // Checkiing if the user survives the attack
